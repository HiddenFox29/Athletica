...
- асинхронное взаимодействие через события (event-driven architecture — событийно-ориентированная архитектура) для уведомлений, аналитики, социальных обновлений, рекомендаций и обработки телеметрии устройств.
...
flowchart TB
    ...
    Device --> Training
    Device --> Analytics[Analytics Domain]
...
Результаты используются Analytics Domain для анализа и формирования рекомендаций.

### 7.2 Социальное взаимодействие
После завершения тренировки пользователь может опубликовать результат.  
Social Domain обрабатывает публикацию, а Notification Domain уведомляет друзей и подписчиков о новом достижении.

### 7.3 Персональные рекомендации
Analytics Domain анализирует историю тренировок, активность пользователя и статистику поведения.  
