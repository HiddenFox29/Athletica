...
Другие инфраструктурные компоненты системы, такие как PostgreSQL, Object Storage (S3-compatible), Cache (кэш) и Monitoring / Logging / Tracing, подробно раскрываются в информационном, инфраструктурном и security представлениях и не детализируются в функциональном представлении как самостоятельные бизнес-домены.
...
- медиа-данные, файлы и резервные копии хранятся в Object Storage (S3-compatible);
- часто читаемые данные могут кэшироваться в Cache (кэш).
...
- для ускорения чтения может использоваться Cache (кэш), не влияющий на транзакционную консистентность доменов.
...
- active-active между регионами (оба региона активны);
...
- устройства — через NGINX и далее в Device Integration Domain.
...
flowchart TB
    ...
    APIGateway[API Gateway] --> Internal[Internal Trusted Zone]
