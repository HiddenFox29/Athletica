...
- транзакционные данные доменов хранятся в отдельных базах данных;
...
- **Databases** — отдельные базы данных доменов по стратегии Database per Service;
...
В каждом регионе также присутствуют инфраструктурные компоненты хранения и наблюдаемости: отдельные базы данных доменов, Object Storage для медиа и резервных копий, Cache для ускорения чтения и Monitoring / Logging / Tracing для диагностики и контроля системы. Это позволяет не смешивать транзакционные, объектные и аналитические данные и обеспечивает управляемость системы при росте нагрузки.
...
| Databases | 99.99% | ≤ 10 минут | ≤ 1 минута | транзакционные данные доменов |
