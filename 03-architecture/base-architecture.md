...
- внешний пользовательский и интеграционный трафик сначала проходит через NGINX (reverse proxy — обратный прокси и балансировщик нагрузки) как ingress (входной контур), после чего пользовательские API-запросы направляются в API Gateway (шлюз API), а интеграционный трафик устройств — в Device Integration Domain;
...
- **API Gateway** — слой публикации и маршрутизации внешних клиентских API (mobile и web);
- **NGINX** — входной ingress, reverse proxy и балансировщик нагрузки для пользовательского и интеграционного трафика;
...
- внешний клиентский API через API Gateway, опубликованный через NGINX как ingress;
- внутренние межсервисные API через внутренний доверенный контур;
...
Между регионами используется модель active-active (оба региона активны), при которой пользовательский трафик может обслуживаться в любом из регионов. Внутри каждого региона используется модель основной/резервный ЦОД с резервным standby-контуром (резервный контур в готовности) для быстрого failover (переключение при сбое) на уровне инфраструктуры.
...
- используется DNS для межрегиональной маршрутизации, NGINX как ingress-балансировщик нагрузки и API Gateway как слой клиентских API.
...
- оркестратор Kubernetes (платформа оркестрации контейнеров) управляет восстановлением, NGINX перенаправляет входящий трафик на доступные экземпляры, а API Gateway маршрутизирует клиентские запросы в соответствующие домены.
...
- входной пользовательский и интеграционный трафик проходит через NGINX, а клиентские API публикуются через API Gateway;
...
- NGINX как отказоустойчивый входной контур и API Gateway как отказоустойчивый слой клиентских API;
...
flowchart TB
    ...
    subgraph R1_EDGE[Регион 1 — входной контур]
        N1[NGINX ingress — Регион 1]
        APIGW1[API Gateway — Регион 1]
    end

    subgraph R2_EDGE[Регион 2 — входной контур]
        N2[NGINX ingress — Регион 2]
        APIGW2[API Gateway — Регион 2]
    end
    ...
    N1 --> DeviceIntegration1
    N2 --> DeviceIntegration2
    ...
    subgraph R1[Регион 1]
        ...
        subgraph R1DC2[ЦОД 2 — резервный standby контур в готовности]
            Auth1b[Auth Domain]
    ...
    subgraph R2[Регион 2]
        ...
        subgraph R2DC4[ЦОД 4 — резервный standby контур в готовности]
            Auth2b[Auth Domain]
    ...
    APIGW1 --> R1_ENTRY[Регион 1 active-active оба региона активны]
    APIGW2 --> R2_ENTRY[Регион 2 active-active оба региона активны]
...
Между регионами используется active-active модель (оба региона активны): оба региона обслуживают пользовательский трафик и участвуют в работе системы. Это позволяет:
...
- резервный ЦОД, используемый как standby-контур (резервный контур в готовности) для failover внутри региона.
...
При отказе всего региона трафик перенаправляется в другой регион с помощью Geo Routing / DNS. После этого региональный NGINX принимает внешний трафик, а региональный API Gateway направляет клиентские API-запросы в доступные домены своего региона.
...
- критичные компоненты (Auth, RabbitMQ) — обеспечивают доступ и целостность данных;
...
Это соответствует стратегии cost-aware scaling (масштабирование с учётом стоимости инфраструктуры и роста нагрузки).
...
|----------|-----|-----|-----|-----------|
| Geo Routing / DNS layer (слой географической маршрутизации и DNS) | 99.99% | ≤ 5 минут | 0 | критично для межрегионального переключения |
| NGINX | 99.9% | ≤ 5 минут | 0 | критичная точка входа и балансировка трафика |
| API Gateway (региональный слой клиентских API) | 99.9% | ≤ 5 минут | 0 | слой клиентских API и маршрутизации внешних запросов |
...
Архитектура сознательно спроектирована как полная multi-region модель (развёртывание в нескольких регионах) с явным отражением всех взаимодействующих доменов, чтобы избежать скрытых компонентов и обеспечить прозрачность для проектирования, эксплуатации и защиты решения.
...
Выбор PostgreSQL, RabbitMQ, NGINX и API Gateway обусловлен тем, что такое разделение ролей обеспечивает надёжность, зрелость экосистемы и соответствие требованиям системы без избыточного усложнения инфраструктурного ingress-слоя.
