...
Если регистрация не может быть выполнена (например, пользователь уже существует или данные некорректны), Auth Domain отклоняет запрос и возвращает обобщённую ошибку без раскрытия причины.
...
Если один из доменов (например, Profile Domain) недоступен или произошла ошибка при обработке запроса, система возвращает ошибку недоступности сервиса.
